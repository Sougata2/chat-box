...
function NewGroupMemberSelector({
  ...
  const [isNewGroupFormOpen, setIsNewGroupFormOpen] = useState<boolean>(false);

  const openNewGroupFormWindow = () => setIsNewGroupFormOpen(true);
  const closeNewGroupFormWindow = () => setIsNewGroupFormOpen(false);
    ...
    <>
      {isNewGroupFormOpen && (
        <NewGroupForm
          ...
          closeWindow={closeNewGroupFormWindow}
        />
      )}
      {!isNewGroupFormOpen && (
        <div className="h-full flex flex-col min-h-0">
          ...
          {selectedContacts.length > 0 && (
            <div className="mx-auto sticky bottom-4 z-10 w-[95%] bg-white rounded-xl flex justify-center py-3">
              <button
                onClick={openNewGroupFormWindow}
